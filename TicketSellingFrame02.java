...
public class TicketSellingFrame02 extends JFrame implements ActionListener 
...
ResultSet rsCH=stmt.executeQuery("select count(Available)  AS  SeatNO  from seatdetailsbus02 where Available=0; ");  
...
{     for(int i=1;i<=40;i++)
	...
	{
	String query = "UPDATE seatdetailsbus02 SET Available=1 WHERE SeatNO="+i+";";
...
for(; i < 7; i++)
		{   
			...
			{				Statement stmt=con.createStatement();  
				...
				while(rsCH.next())  
			{
				TempInfo=rsCH.getString("BUS_Starting_Time02");
				TicketInfoLevel[i+7].setText(TempInfo);
...
//avail seat genarate
    ...
    // String query1 = "UPDATE ticketinfolevelinfodatabase SET AvBUsSeat01="+AvailbusSeat+" WHERE TLid=1;";
	 ...
	 // String query = "UPDATE seatdetailsbus01 SET Available=false WHERE SeatNO="+x.getText()+";";
	//Statement stmt=con.createStatement(); 
    String query2 = "UPDATE ticketinfolevelinfodatabase SET AvBUsSeat02="+AvailbusSeat+" WHERE TLid=1;";
...
con.close();  
	...
	ResultSet rsCH=stmt.executeQuery("select * from seatdetailsbus02 where SeatNO="+x.getText()+";");  
...
con.close();  
	...
	{ try
    {
      ...
      String query = "UPDATE seatdetailsbus02 SET Available=0 WHERE SeatNO="+x.getText()+";";
   ...
   preparedStmt.executeUpdate();
	...
	{      
		...
		{            setVisible(false);
			TicketSellingFrame03 Tf4 =new TicketSellingFrame03();

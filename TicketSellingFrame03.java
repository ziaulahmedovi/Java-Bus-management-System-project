...
public class TicketSellingFrame03 extends JFrame implements ActionListener 
...
ResultSet rsCH=stmt.executeQuery("select count(Available)  AS  SeatNO  from seatdetailsbus03 where Available=0; ");  
...
con.close();  
    ...
    {   
     ...
     // String query = "UPDATE seatdetailsbus04 SET Available=false WHERE SeatNO="+x.getText()+";";
	Statement stmt=con.createStatement(); 
  //id genarate
...
{   redseat=0;
	...
	{
	String query = "UPDATE seatdetailsbus03 SET Available=1 WHERE SeatNO="+i+";";
...
for(; i < 7; i++)
		{   
			...
			{				Statement stmt=con.createStatement();  
				...
				while(rsCH.next())  
			{
				TempInfo=rsCH.getString("BUS_Starting_Time03");
				TicketInfoLevel[i+7].setText(TempInfo);
...
//avail seat genarate
    ...
    // String query1 = "UPDATE ticketinfolevelinfodatabase SET AvBUsSeat01="+AvailbusSeat+" WHERE TLid=1;";
	 ...
	 // String query = "UPDATE seatdetailsbus01 SET Available=false WHERE SeatNO="+x.getText()+";";
	//Statement stmt=con.createStatement(); 
    String query2 = "UPDATE ticketinfolevelinfodatabase SET AvBUsSeat03="+AvailbusSeat+" WHERE TLid=1;";
...
con.close();  
	...
	ResultSet rsCH=stmt.executeQuery("select * from seatdetailsbus03 where SeatNO="+x.getText()+";");  
...
con.close();  
	...
	{ try
    {
      ...
      String query = "UPDATE seatdetailsbus03 SET Available=0 WHERE SeatNO="+x.getText()+";";
   ...
   preparedStmt.executeUpdate();
	...
	{      
		...
		{              setVisible(false);
			TicketSellingFrame04 Tf4 =new TicketSellingFrame04();
